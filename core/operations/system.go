...
func NewSystem(o Options) *System {
	...
	system.initializeServer()             //初始化一个服务
	system.initializeHealthCheckHandler() //构建服务句柄（/healthz 调用地址）
	system.initializeLoggingHandler()     //构建服务请求日志规格句柄（/logspec）
	system.initializeMetricsProvider()    //构建metric选项支持  好像都选择prometheus模式 可以从（/metrics）地址调用查看
