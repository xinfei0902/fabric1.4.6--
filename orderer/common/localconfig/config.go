...
func Load() (*TopLevel, error) {
	config := viper.New()
	coreconfig.InitViper(config, "orderer")   //初始化  存下配置文件绝对路径
	config.SetEnvPrefix(Prefix)               //设置 环境变量字首 比如 ORDERER_GENERAL_LOCALMSPID=OrdererMSP
	config.AutomaticEnv()                     //检测所有环境变量
	replacer := strings.NewReplacer(".", "_") //设置规则模板  把.变为_ 比如配置文件orderer.general  orderer_general
	config.SetEnvKeyReplacer(replacer)        //把规则赋viper结构中

	if err := config.ReadInConfig(); err != nil { //读取配置文件orderer.yaml 内容赋值viper.config中 待使用
		return nil, fmt.Errorf("Error reading configuration: %s", err)
	}

	var uconf TopLevel                                                       //其实是配置文件的结构对象
	if err := viperutil.EnhancedExactUnmarshal(config, &uconf); err != nil { //转结构 具体内容还得细看 没看懂
		return nil, fmt.Errorf("Error unmarshaling config into struct: %s", err)
	}

	uconf.completeInitialization(filepath.Dir(config.ConfigFileUsed())) //检测uconf配置结构是否空缺 如果空缺从默认Defults(当前文件No 214 line)中赋值
	return &uconf, nil
}

//completeInitialization 检测uconf配置结构是否空缺 如果空缺从默认Defults(当前文件No 214 line)中赋值
