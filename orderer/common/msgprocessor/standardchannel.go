...
func (s *StandardChannel) ProcessNormalMsg(env *cb.Envelope) (configSeq uint64, err error) {
	oc, ok := s.support.OrdererConfig() //获取指定通道 orderer配置信息  实现：common\channelconfig\bundlesource.go No.79 line
	if !ok {
		logger.Panicf("Missing orderer config")
	}
	if oc.Capabilities().ConsensusTypeMigration() { //true   Capabilities() 获取普通通道Capabilities 实现common\channelconfig\orderer.go No.189 line  实现：ConsensusTypeMigration() ===common\capabilities\orderer.go No.80 line
		if oc.ConsensusState() != orderer.ConsensusType_STATE_NORMAL { //相等 //实现 fabric\common\channelconfig\orderer.go No.157 line
			return 0, errors.WithMessage(
				ErrMaintenanceMode, "normal transactions are rejected")
		}
	}
	configSeq = s.support.Sequence()
	err = s.filters.Apply(env) //规则循环验证 空的  大小 签名 实现 fabric\orderer\common\msgprocessor\ sizefilter.go检测消息大小  sigfliter.go检测消息签名 expiration.go检测签名证书是否在有效期内
	return
}
