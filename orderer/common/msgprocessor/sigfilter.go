...
func (sf *SigFilter) Apply(message *cb.Envelope) error {
	ordererConf, ok := sf.support.OrdererConfig() //获取通道orderer配置
	if !ok {
	...
	policy, ok := sf.support.PolicyManager().GetPolicy(policyName) //根据策略名称获取指定策略配置  实现  fabric\common\policies\policy.go No.274 line
	if !ok {
		return fmt.Errorf("could not find policy %s", policyName)
	}

	err = policy.Evaluate(signedData) // 对签名数据进行验签 实现  fabric\common\policies\policy.go No.258 line
	if err != nil {
