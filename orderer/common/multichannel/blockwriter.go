...
func (bw *BlockWriter) CreateNextBlock(messages []*cb.Envelope) *cb.Block {
	previousBlockHash := bw.lastBlock.Header.Hash() //最后一个块hash当作新块的 前一块hash
	...
	block := cb.NewBlock(bw.lastBlock.Header.Number+1, previousBlockHash) //创建块结构
...
func (bw *BlockWriter) WriteConfigBlock(block *cb.Block, encodedMetadataValue []byte) {
	ctx, err := utils.ExtractEnvelope(block, 0) //从块中data.data[0]获取交易信息（信封envelope）
	if err != nil {
	...
	switch chdr.Type { //根据交易信息类型选择执行操作
	case int32(cb.HeaderType_ORDERER_TRANSACTION): //进入这个函数 又是交易类型 那只能是创建通道 新建通道配置块
	...
	case int32(cb.HeaderType_CONFIG):
		...
		err = bw.support.Validate(configEnvelope) // 实现 fabric\common\configtx\validator.go No.160 line
		if err != nil {
			logger.Panicf("Told to write a config block with new config, but could not apply it: %s", err)
		}

		bundle, err := bw.support.CreateBundle(chdr.ChannelId, configEnvelope.Config) //实现 fabric\orderer\common\multichannel\registrar.go No.352 line
		if err != nil {
