...
func newChainSupport(
	...
	lastBlock := blockledger.GetBlock(ledgerResources, ledgerResources.Height()-1)        //获取最后一个块
	metadata, err := utils.GetMetadataFromBlock(lastBlock, cb.BlockMetadataIndex_ORDERER) //获取块中metadata{}数据
	...
	cs.Chain, err = consenter.HandleChain(cs, metadata) //实现 如果是kafka orderer/consensus/kafka/consenter.go No.86 line
	if err != nil {
