...
func ConfigBlock(reader blockledger.Reader) *cb.Block {
	...
	index, err := utils.GetLastConfigIndexFromBlock(lastBlock) //取最后一个块的metadata[1]的值
	if err != nil {
...
func (r *Registrar) Initialize(consenters map[string]consensus.Consenter) {
	...
	existingChains := r.ledgerFactory.ChainIDs() //获取通道 实现在外层 common\ledger\blockledger\file\factory.go No.56 line

	for _, chainID := range existingChains {
		rl, err := r.ledgerFactory.GetOrCreate(chainID) //创建或获取账本
		if err != nil {
			logger.Panicf("Ledger factory reported chainID %s but could not retrieve it: %s", chainID, err)
		}
		configTx := configTx(rl) //获取配置
		if configTx == nil {
			logger.Panic("Programming error, configTx should never be nil here")
		}
		ledgerResources := r.newLedgerResources(configTx)        //根据配置 检测策略配置规则 读取账本构建新的账本
		chainID := ledgerResources.ConfigtxValidator().ChainID() //从配置管理获取通道名称 ConfigtxValidator()实现==common\channelconfig\bundle.go No.74 line

		if _, ok := ledgerResources.ConsortiumsConfig(); ok { //通道联盟配置是否存在
			...
			iter, pos := rl.Iterator(&ab.SeekPosition{Type: &ab.SeekPosition_Oldest{Oldest: &ab.SeekOldest{}}}) //返回：通过类型构建检索块范围配置,开始区块高度  实现common\ledger\blockledger\file\impl.go No.66 line
			defer iter.Close()
			if pos != uint64(0) {
				logger.Panicf("Error iterating over system channel: '%s', expected position 0, got %d", chainID, pos)
			}
			genesisBlock, status := iter.Next() //块,状态 实现common\ledger\blockledger\file\impl.go No.46 line
			...
			defer chain.start() //启动线程 创建主题 创建生产者消费者 保持数据连接 实现 fabric\orderer\consensus\kafka\chain.go No.146 line
		} else {
...
func (r *Registrar) BroadcastChannelSupport(msg *cb.Envelope) (*cb.ChannelHeader, bool, *ChainSupport, error) {
	chdr, err := utils.ChannelHeader(msg) //从交易中提取channelheade信息
	if err != nil {
		return nil, false, nil, fmt.Errorf("could not determine channel ID: %s", err)
	}

	cs := r.GetChain(chdr.ChannelId) //根据通道名称 获取此通道支持项
	...
	isConfig := false
	switch cs.ClassifyMsg(chdr) { //根据头部信息 判断此交易类型（是交易还是配置更新等）实现 fabric\orderer\common\msgprocessor\standardchannel.go No.76 line
...
func (r *Registrar) newLedgerResources(configTx *cb.Envelope) *ledgerResources {
	...
	bundle, err := channelconfig.NewBundle(chdr.ChannelId, configEnvelope.Config) //创建策略管理 通道配置 通道配置管理
	if err != nil {
		logger.Panicf("Error creating channelconfig bundle: %s", err)
	}

	checkResourcesOrPanic(bundle) //对策略配置检测
