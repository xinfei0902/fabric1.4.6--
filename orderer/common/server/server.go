...
func (bs broadcastSupport) BroadcastChannelSupport(msg *cb.Envelope) (*cb.ChannelHeader, bool, broadcast.ChannelSupport, error) {
	return bs.Registrar.BroadcastChannelSupport(msg) // 实现 fabric\orderer\common\multichannel\registrar.go No.224 line
}
...
func (s *server) Broadcast(srv ab.AtomicBroadcast_BroadcastServer) error { //广播服务 接收peer广播的信封（envelope）
