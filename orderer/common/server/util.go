...
func createLedgerFactory(conf *config.TopLevel, metricsProvider metrics.Provider) (blockledger.Factory, string) {
	...
	var ld string
	switch conf.General.LedgerType { //基本配置都是file
	case "file":
		ld = conf.FileLedger.Location //zhuangb账本存放路径
		if ld == "" {
			ld = createTempDir(conf.FileLedger.Prefix)
		}
		logger.Debug("Ledger dir:", ld)
		lf = fileledger.New(ld, metricsProvider) //创建 账本存放路径/index/目录 以及基于文件系统快速存储
		...
		createSubDir(ld, fsblkstorage.ChainsDir) //创建 账本存放路径/chains/目录
	case "json":
