...
func NewBlock(seqNum uint64, previousHash []byte) *Block { //构建区块  建议拉取一个块解析内容 对着看 一目了然
	block := &Block{}
	block.Header = &BlockHeader{}            //构建区块头部
	block.Header.Number = seqNum             //当前区块高度为0
	block.Header.PreviousHash = previousHash //前一个块hash
	...
	block.Metadata = &BlockMetadata{Metadata: metadataContents} //一开始有几种类型 元数据都为 ""空
