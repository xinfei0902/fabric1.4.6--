...
func CreateSignedEnvelopeWithTLSBinding(txType common.HeaderType, channelID string, signer crypto.LocalSigner, dataMsg proto.Message, msgVersion int32, epoch uint64, tlsCertHash []byte) (*common.Envelope, error) {
	payloadChannelHeader := MakeChannelHeader(txType, msgVersion, channelID, epoch) //创建payload.header的 channel_header
	payloadChannelHeader.TlsCertHash = tlsCertHash
	var err error
	payloadSignatureHeader := &common.SignatureHeader{}

	if signer != nil {
		payloadSignatureHeader, err = signer.NewSignatureHeader() //创建signature_header
		if err != nil {
			return nil, err
		}
	}
	...
	paylBytes := MarshalOrPanic( //创建payload 包含header和 上面序列化data
		&common.Payload{
	...
	var sig []byte
	if signer != nil {
		sig, err = signer.Sign(paylBytes) //对数据签署
		if err != nil {
			return nil, err
		}
	}

	env := &common.Envelope{ //组装成交易（信封）
		Payload:   paylBytes,
		Signature: sig,
	}
