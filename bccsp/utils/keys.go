...
func PrivateKeyToPEM(privateKey interface{}, pwd []byte) ([]byte, error) {
	// Validate inputs
	if len(pwd) != 0 {
		return PrivateKeyToEncryptedPEM(privateKey, pwd) //如果密码不为空 对pem块加密
	}
	if privateKey == nil {
	...
	case *ecdsa.PrivateKey:
		...
		oidNamedCurve, ok := oidFromNamedCurve(k.Curve) //获取那种椭圆曲线 一般时 p-256
		if !ok {
		...
		return pem.EncodeToMemory( //把类型以 ---BEGIN PRIVATE KEY -----   Bytes  ----END PRIVATE KEY----组成pem块
			&pem.Block{
				Type:  "PRIVATE KEY",
				Bytes: pkcs8Bytes,
			},
...
func PEMtoPrivateKey(raw []byte, pwd []byte) (interface{}, error) {
	...
	block, _ := pem.Decode(raw) //解析私钥文件 获取私钥类型 内容等
	if block == nil {
	...
	if x509.IsEncryptedPEMBlock(block) { //返回私钥 PEM块是否是用密码加密了的
		if len(pwd) == 0 {
	...
	cert, err := DERToPrivateKey(block.Bytes) //解析asn.1 der编码私钥
	if err != nil {
		return nil, err
	}
	return cert, err
...
func DERToPublicKey(raw []byte) (pub interface{}, err error) {
	...
	key, err := x509.ParsePKIXPublicKey(raw) //解析一个DER编码的公钥。这些公钥一般在以"BEGIN PUBLIC KEY"出现的PEM块中
