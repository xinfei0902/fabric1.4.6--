...
func GetDefault() bccsp.BCCSP {
	if defaultBCCSP == nil { //没有InitFactories（）则进入
		logger.Debug("Before using BCCSP, please call InitFactories(). Falling back to bootBCCSP.")
		bootBCCSPInitOnce.Do(func() {
			var err error
			f := &SWFactory{}
			bootBCCSP, err = f.Get(GetDefaultOpts()) //GetDefaultOpts()构建默认sw配置选项工厂
			if err != nil {
...
func GetBCCSP(name string) (bccsp.BCCSP, error) { //根据指定name 获取bccsp实例
...
func initBCCSP(f BCCSPFactory, config *FactoryOpts) error {
	csp, err := f.Get(config) //根据配置构建bccsp实例 具体看是什么实现 比如f = SWFactory{} 那么实现=同级目录 fabric\bccsp\factory\swfactory.go No.38 line
	if err != nil {
		return errors.Errorf("Could not initialize BCCSP %s [%s]", f.Name(), err)
	}

	logger.Debugf("Initialize BCCSP [%s]", f.Name())
	bccspMap[f.Name()] = csp //构建完bccsp实例 存入全局变量bccspMap
	return nil
}
