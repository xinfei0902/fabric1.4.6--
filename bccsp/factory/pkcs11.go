...
func setFactories(config *FactoryOpts) error {
	...
	// PKCS11-Based BCCSP
	if config.Pkcs11Opts != nil { //pkcs11
		f := &PKCS11Factory{}
		err := initBCCSP(f, config) //构建新的bccsp实例
		if err != nil {
...
func GetBCCSPFromOpts(config *FactoryOpts) (bccsp.BCCSP, error) {
	...
	csp, err := f.Get(config) //实现 fabric\bccsp\factory\pkcs11factory.go No.41 line
	if err != nil {
