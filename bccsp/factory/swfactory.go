...
func (f *SWFactory) Get(config *FactoryOpts) (bccsp.BCCSP, error) {
	...
	if swOpts.Ephemeral == true { //临时
		ks = sw.NewDummyKeyStore()
	} else if swOpts.FileKeystore != nil { //密钥是长久
	...
	return sw.NewWithParams(swOpts.SecLevel, swOpts.HashFamily, ks) //构建新的bccsp实例 参数 例如（安全等级256，sha2，ks）
}
