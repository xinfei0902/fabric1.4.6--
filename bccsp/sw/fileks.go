...
func (ks *fileBasedKeyStore) Init(pwd []byte, path string, readOnly bool) error {
	...
	err := ks.createKeyStoreIfNotExists() //如果不存在 创建keystroe
	if err != nil {
		return err
	}

	err = ks.openKeyStore() //判断是否已经完成keystore  设置bool变量isOpen状态 （此状态全局）
	if err != nil {
		return err
	}
...
func (ks *fileBasedKeyStore) GetKey(ski []byte) (bccsp.Key, error) {
	...
	case "sk":
		...
		key, err := ks.loadPrivateKey(hex.EncodeToString(ski)) //加载并解析文件存储的私钥
		if err != nil {
...
func (ks *fileBasedKeyStore) StoreKey(k bccsp.Key) (err error) { //以文件形式存储key
	if ks.readOnly {
	...
	switch k.(type) {
	case *ecdsaPrivateKey: //椭圆算法私钥
		kk := k.(*ecdsaPrivateKey)

		err = ks.storePrivateKey(hex.EncodeToString(k.SKI()), kk.privKey) //以ski为名_sk为后缀文件存放 私钥经过asn.1der编码 内容嵌套进 ---BEGIN type --- ***  ---END type--组成pem块
		if err != nil {
...
func (ks *fileBasedKeyStore) searchKeystoreForSKI(ski []byte) (k bccsp.Key, err error) { //在存放keystore路径下找到与传进来的ski匹配的私钥
	...
	for _, f := range files {
		...
		key, err := utils.PEMtoPrivateKey(raw, ks.pwd) //pem块转私钥
		if err != nil {
			continue
		}
...
func (ks *fileBasedKeyStore) getSuffix(alias string) string {
	files, _ := ioutil.ReadDir(ks.path)
	for _, f := range files {
		if strings.HasPrefix(f.Name(), alias) { //是否前缀一直
...
func (ks *fileBasedKeyStore) storePrivateKey(alias string, privateKey interface{}) error {
	rawKey, err := utils.PrivateKeyToPEM(privateKey, ks.pwd) //私钥经过asn.1 der编码转换pem块  具体操做可以看一下私钥文件内容
	if err != nil {
		logger.Errorf("Failed converting private key to PEM [%s]: [%s]", alias, err)
		return err
	}

	err = ioutil.WriteFile(ks.getPathForAlias(alias, "sk"), rawKey, 0600) //把 pem块写的文件中 可以参考 msp/keystore目录下 后缀为 _sk文件
	if err != nil {
...
func (ks *fileBasedKeyStore) loadPrivateKey(alias string) (interface{}, error) {
	path := ks.getPathForAlias(alias, "sk") //拼接私钥文件名称  ****_sk
	...
	privateKey, err := utils.PEMtoPrivateKey(raw, ks.pwd) //解析asn.1 der编码私钥 这个过程与keystore把私钥存放文件相反
	if err != nil {
...
func (ks *fileBasedKeyStore) createKeyStoreIfNotExists() error {
	...
	ksPath := ks.path
	missing, err := utils.DirMissingOrEmpty(ksPath) //判断 msp/keystore路径是否存在

	if missing {
		logger.Debugf("KeyStore path [%s] missing [%t]: [%s]", ksPath, missing, utils.ErrToString(err))

		err := ks.createKeyStore() //创建keystore目录
		if err != nil {
