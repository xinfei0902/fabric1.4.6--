...
func (csp *CSP) KeyGen(opts bccsp.KeyGenOpts) (k bccsp.Key, err error) { //创建key 不是临时key 存储key 长期一般存放文件
	// Validate arguments
	if opts == nil {
		return nil, errors.New("Invalid Opts parameter. It must not be nil.")
	}

	keyGenerator, found := csp.KeyGenerators[reflect.TypeOf(opts)] //反射类型 可以根据类型指定采用那种算法去生成key
	if !found {
		return nil, errors.Errorf("Unsupported 'KeyGenOpts' provided [%v]", opts)
	}

	k, err = keyGenerator.KeyGen(opts) //创建公私密钥对 如果是ecds椭圆曲线算法 实现为fabric\bccsp\sw\keygen.go No.33 line 其他算法 在同文件其他行
	if err != nil {
		return nil, errors.Wrapf(err, "Failed generating key with opts [%v]", opts)
	}

	// If the key is not Ephemeral, store it.
	if !opts.Ephemeral() { //密钥不是临时的
		// Store the key
		err = csp.ks.StoreKey(k) //存储密钥
		if err != nil {
...
func (csp *CSP) KeyDeriv(k bccsp.Key, opts bccsp.KeyDerivOpts) (dk bccsp.Key, err error) {
	...
	k, err = keyDeriver.KeyDeriv(k, opts) //根据不同算法  实现fabric\bccsp\sw\keyderiv.go
	if err != nil {
...
func (csp *CSP) GetKey(ski []byte) (k bccsp.Key, err error) {
	k, err = csp.ks.GetKey(ski) //根据ski 拼接私钥存储位置 然后获取私钥
	if err != nil {
...
func (csp *CSP) Sign(k bccsp.Key, digest []byte, opts bccsp.SignerOpts) (signature []byte, err error) {
	...
	signature, err = signer.Sign(k, digest, opts) //k是那种算法 去哪里文件找实现
	if err != nil {
