...
func makeImplicitMetaPolicy(subPolicyName string, rule cb.ImplicitMetaPolicy_Rule) *cb.Policy { //块中策略policy 可以参考区块配置内容对照
	return &cb.Policy{
		Type: int32(cb.Policy_IMPLICIT_META), //3
		Value: utils.MarshalOrPanic(&cb.ImplicitMetaPolicy{
			Rule:      rule,          //ANY
			SubPolicy: subPolicyName, //Admins
		}),
	}
}
...
func ImplicitMetaMajorityPolicy(policyName string) *StandardConfigPolicy { //构建区块配置中policy  是channel_group.policies 模块中的
	return &StandardConfigPolicy{
		key:   policyName, //Admins
