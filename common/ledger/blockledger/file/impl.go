...
func (fl *FileLedger) Iterator(startPosition *ab.SeekPosition) (blockledger.Iterator, uint64) { //这块用来获取块 像通过命令 peer channel fetch newest | oldest |config | number
	var startingBlockNumber uint64
	switch start := startPosition.Type.(type) {
	case *ab.SeekPosition_Oldest:
		startingBlockNumber = 0
	case *ab.SeekPosition_Newest:
		info, err := fl.blockStore.GetBlockchainInfo() //获取块的信息（高度，前一块hash ,当前块hash）  实现common\ledger\blkstorage\fsblkstorage\fs_blockstore.go No.63 line
		if err != nil {
			logger.Panic(err)
		}
	...
	iterator, err := fl.blockStore.RetrieveBlocks(startingBlockNumber) //new iterator（检索块）范围从startingBlockNumber到最后一个块 实现common\ledger\blkstorage\fsblkstorage\fs_blockstore.go No.68 line
	if err != nil {
		return &blockledger.NotFoundErrorIterator{}, 0
	}
