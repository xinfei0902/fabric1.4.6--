...
func (dbInst *DB) Open() {
	...
	if dbInst.dbState == opened { //dbState==0  opened==1
		return
	}
	dbOpts := &opt.Options{}
	...
	var err error
	var dirEmpty bool
	if dirEmpty, err = util.CreateDirIfMissing(dbPath); err != nil { //创建路径
		panic(fmt.Sprintf("Error creating dir if missing: %s", err))
	}
	dbOpts.ErrorIfMissing = !dirEmpty //false
