	...
	return func(f reflect.Kind, t reflect.Kind, data interface{}) (interface{}, error) {
		...
		case reflect.Map:
			...
			case ok && fileName != "":
				...
				for len(bytes) > 0 {
					var block *pem.Block
					block, bytes = pem.Decode(bytes) //解析证书 ---BEGIN 后面是type ---- ***** ----END type------
					if block == nil {
						break
					}
