...
func getConfig() (*Config, error) {
	...
	if *genConfigFile != nil { //获取 命令行 --config=参数
	...
	config := &Config{}
	err := yaml.Unmarshal([]byte(configData), &config) //字节反序列化配置结构
	if err != nil {
...
func extend() {
	config, err := getConfig() //获取创建证书配置文件（crypto-config）
	if err != nil {
...
func extendPeerOrg(orgSpec OrgSpec) { //在已有网络中 扩展 组织下或者节点证书
	orgName := orgSpec.Domain
	orgDir := filepath.Join(*inputDir, "peerOrganizations", orgName)
	if _, err := os.Stat(orgDir); os.IsNotExist(err) { //检测组织是否存在
...
func extendOrdererOrg(orgSpec OrgSpec) { //在已有的网络 扩展orderer证书
	orgName := orgSpec.Domain
...
func generate() {
	...
	for _, orgSpec := range config.PeerOrgs {
		err = renderOrgSpec(&orgSpec, "peer") //检测配置补充 节点规则
		if err != nil {
			fmt.Printf("Error processing peer configuration: %s", err)
			os.Exit(-1)
		}
		generatePeerOrg(*outputDir, orgSpec) //创建节点证书 outputDir:命令行没有指定 默认本地crypto-config文件夹
	}
	...
	for _, orgSpec := range config.OrdererOrgs {
		...
		generateOrdererOrg(*outputDir, orgSpec) //创建orderer证书 与生成节点类似
	}
}
...
func parseTemplateWithDefault(input, defaultInput string, data interface{}) (string, error) {
	...
	if len(input) == 0 {
		input = defaultInput //以 "${prefix}+${index} 组装默认  比如 peer+0=peer0
	}
...
func renderNodeSpec(domain string, spec *NodeSpec) error {
	...
	cn, err := parseTemplateWithDefault(spec.CommonName, defaultCNTemplate, data) //如果配置CommonName为空组 根据模板解析组装默认名  比如 peer0.org1.example.com等
	if err != nil {
		return err
	}

	spec.CommonName = cn // 例如 peer0.org1.example.com
	data.CommonName = cn
...
func renderOrgSpec(orgSpec *OrgSpec, prefix string) error {
	...
	for i := 0; i < orgSpec.Template.Count; i++ {
		...
		hostname, err := parseTemplateWithDefault(orgSpec.Template.Hostname, defaultHostnameTemplate, data) //如果配置Hostname为空组 根据模板解析组装默认名  比如 orderer0,peer0等
		if err != nil {
			return err
		}
...
func generatePeerOrg(baseDir string, orgSpec OrgSpec) {
	...
	orgName := orgSpec.Domain //  比如 org1.example.com
	...
	err = msp.GenerateVerifyingMSP(mspDir, signCA, tlsCA, orgSpec.EnableNodeOUs) //创建msp下
	if err != nil {
		fmt.Printf("Error generating MSP for org %s:\n%v\n", orgName, err)
		os.Exit(1)
	}

	generateNodes(peersDir, orgSpec.Specs, signCA, tlsCA, msp.PEER, orgSpec.EnableNodeOUs) //创建 peers下证书
	...
	generateNodes(usersDir, users, signCA, tlsCA, msp.CLIENT, orgSpec.EnableNodeOUs) //创建 users下证书

	// copy the admin cert to the org's MSP admincerts
	if !orgSpec.EnableNodeOUs { //当证书配置文件 EnableNodeOUs没有设置为true 会在msp/admincerts考入admin用户证书
...
func generateNodes(baseDir string, nodes []NodeSpec, signCA *ca.CA, tlsCA *ca.CA, nodeType int, nodeOUs bool) {
	...
	for _, node := range nodes {
		nodeDir := filepath.Join(baseDir, node.CommonName) //  dir=:****/peers/peer*.org1.example.com
		if _, err := os.Stat(nodeDir); os.IsNotExist(err) {
			currentNodeType := nodeType //2
			if node.isAdmin && nodeOUs {
				currentNodeType = msp.ADMIN
			}
			err := msp.GenerateLocalMSP(nodeDir, node.CommonName, node.SANS, signCA, tlsCA, currentNodeType, nodeOUs) //创建msp 和 tls
			if err != nil {
