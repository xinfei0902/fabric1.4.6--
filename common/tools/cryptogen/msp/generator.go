...
func GenerateVerifyingMSP(baseDir string, signCA *ca.CA, tlsCA *ca.CA, nodeOUs bool) error {
	...
	err := createFolderStructure(baseDir, false) //创建msp下目录
	if err == nil {
		// the signing CA certificate goes into cacerts
		err = x509Export(filepath.Join(baseDir, "cacerts", x509Filename(signCA.Name)), signCA.SignCert) //对具体证书内容加头和尾 也就是 ----BEGIN ***----- ***证书内容*** ----END ***----
		if err != nil {
			return err
		}
		// the TLS CA certificate goes into tlscacerts
		err = x509Export(filepath.Join(baseDir, "tlscacerts", x509Filename(tlsCA.Name)), tlsCA.SignCert) //同上
		if err != nil {
			return err
		}
	}
	...
	// generate config.yaml if required
	if nodeOUs {
		exportConfig(baseDir, "cacerts/"+x509Filename(signCA.Name), true) //创建证书msp下config.yaml文件
	}
