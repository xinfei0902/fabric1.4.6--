...
func NewCA(baseDir, org, name, country, province, locality, orgUnit, streetAddress, postalCode string) (*CA, error) {
	...
	if err == nil {
		priv, signer, err := csp.GeneratePrivateKey(baseDir) //获取私钥存放于keystore中（keystore目录下 和全局csp变量） 和创建签名用私钥
		response = err
		if err == nil {
			// get public signing certificate
			ecPubKey, err := csp.GetECPublicKey(priv) //获取公钥
			response = err
			if err == nil {
				template := x509Template() // x509cert 模板
				//this is a CA
				template.IsCA = true
