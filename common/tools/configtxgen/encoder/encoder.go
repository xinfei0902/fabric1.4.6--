...
func addPolicies(cg *cb.ConfigGroup, policyMap map[string]*genesisconfig.Policy, modPolicy string) error {
	for policyName, policy := range policyMap { //处理configtx.yaml中 Organizations 的policies是Signature 其他模块都是ImplicitMeta
		switch policy.Type {
		case ImplicitMetaPolicyType:
			imp, err := policies.ImplicitMetaFromString(policy.Rule) //返回策略
			if err != nil {
		...
		case SignaturePolicyType:
			sp, err := cauthdsl.FromString(policy.Rule) //签名策略
			if err != nil {
...
func addImplicitMetaPolicyDefaults(cg *cb.ConfigGroup) {
	addPolicy(cg, policies.ImplicitMetaMajorityPolicy(channelconfig.AdminsPolicyKey), channelconfig.AdminsPolicyKey) //构建区块配置中policy  是channel_group.policies.Admin 模块中的
	addPolicy(cg, policies.ImplicitMetaAnyPolicy(channelconfig.ReadersPolicyKey), channelconfig.AdminsPolicyKey)     //构建区块配置中policy  是channel_group.policies.Readers 模块中的
	addPolicy(cg, policies.ImplicitMetaAnyPolicy(channelconfig.WritersPolicyKey), channelconfig.AdminsPolicyKey)     //构建区块配置中policy  是channel_group.policies.Writers 模块中的
}
...
func addSignaturePolicyDefaults(cg *cb.ConfigGroup, mspID string, devMode bool) { //添加策略 参考channel_groups.Orderer.groups,OrdererOrg.policies
	if devMode {
...
func NewChannelGroup(conf *genesisconfig.Profile) (*cb.ConfigGroup, error) {
	channelGroup := cb.NewConfigGroup() //new 实例 区块配置中channel_group
	if len(conf.Policies) == 0 {
		...
		addImplicitMetaPolicyDefaults(channelGroup) //在区块配置 channel_group.policies 添加策略
	} else {
		if err := addPolicies(channelGroup, conf.Policies, channelconfig.AdminsPolicyKey); err != nil { //添加策略
			return nil, errors.Wrapf(err, "error adding policies to channel group")
		}
	}

	addValue(channelGroup, channelconfig.HashingAlgorithmValue(), channelconfig.AdminsPolicyKey)          //hash算法 参考块配置 channel_group.Values.HashingAlgorithm
	addValue(channelGroup, channelconfig.BlockDataHashingStructureValue(), channelconfig.AdminsPolicyKey) //参考块配置 channel_group.Values.BlockDataHashingStructure
	if conf.Orderer != nil && len(conf.Orderer.Addresses) > 0 {
		addValue(channelGroup, channelconfig.OrdererAddressesValue(conf.Orderer.Addresses), ordererAdminsPolicyName) //参考块配置 channel_group.Values.OrdererAddresses
	}

	if conf.Consortium != "" { //不进入
		addValue(channelGroup, channelconfig.ConsortiumValue(conf.Consortium), channelconfig.AdminsPolicyKey)
	}

	if len(conf.Capabilities) > 0 {
		addValue(channelGroup, channelconfig.CapabilitiesValue(conf.Capabilities), channelconfig.AdminsPolicyKey) ////参考块配置 channel_group.Values.Capabilities
	}

	var err error
	if conf.Orderer != nil {
		channelGroup.Groups[channelconfig.OrdererGroupKey], err = NewOrdererGroup(conf.Orderer) //配置通道下orderer策略规则以及msp
		if err != nil {
	...
	if conf.Application != nil {
		channelGroup.Groups[channelconfig.ApplicationGroupKey], err = NewApplicationGroup(conf.Application) //基本类似同上=====配置通道下Application策略规则以及msp
		if err != nil {
	...
	if conf.Consortiums != nil {
		channelGroup.Groups[channelconfig.ConsortiumsGroupKey], err = NewConsortiumsGroup(conf.Consortiums) //基本类似同上===联盟配置这一块是重点=====配置通道下Consortiums策略规则以及msp
		if err != nil {
...
func NewOrdererGroup(conf *genesisconfig.Orderer) (*cb.ConfigGroup, error) {
	...
	if len(conf.Policies) == 0 {
		...
		addOrdererImplicitMetaPolicyDefaults(ordererGroup) //构建orderer 策略（admin readers writers blockvalidat）
	} else {
		if err := addPolicies(ordererGroup, conf.Policies, channelconfig.AdminsPolicyKey); err != nil { //添加策略
			return nil, errors.Wrapf(err, "error adding policies to orderer group")
		}
	}
	addValue(ordererGroup, channelconfig.BatchSizeValue( //以下的addValue()函数 都参考区块配置 channel_group.groups.Orderer.values 功能很容易明白
	...
	for _, org := range conf.Organizations {
		var err error
		ordererGroup.Groups[org.Name], err = NewOrdererOrgGroup(org) //配置groups下orderer策略
		if err != nil {
...
func NewOrdererOrgGroup(conf *genesisconfig.Organization) (*cb.ConfigGroup, error) {
	mspConfig, err := msp.GetVerifyingMspConfig(conf.MSPDir, conf.ID, conf.MSPType) //根据条件获取msp目录下全部内容 返回msp配置 包含内容
	if err != nil {
		return nil, errors.Wrapf(err, "1 - Error loading MSP configuration for org: %s", conf.Name)
	}
	...
	if len(conf.Policies) == 0 {
		logger.Warningf("Default policy emission is deprecated, please include policy specifications for the orderer org group %s in configtx.yaml", conf.Name)
		addSignaturePolicyDefaults(ordererOrgGroup, conf.ID, conf.AdminPrincipal != genesisconfig.AdminRoleAdminPrincipal) //添加策略
	} else {
		if err := addPolicies(ordererOrgGroup, conf.Policies, channelconfig.AdminsPolicyKey); err != nil { //添加策略
			return nil, errors.Wrapf(err, "error adding policies to orderer org group '%s'", conf.Name)
		}
	}
	//参考 channel_group.groups.Orderer.groups.OrdererOrg.values.msp
	...
	if len(conf.OrdererEndpoints) > 0 { //不进入
...
func New(config *genesisconfig.Profile) *Bootstrapper {
	channelGroup, err := NewChannelGroup(config) //主要就是 configtx.yaml文件配置信息 转成区块配置行 根据配置定下策略规则等
	if err != nil {
...
func (bs *Bootstrapper) GenesisBlockForChannel(channelID string) *cb.Block {
	return genesis.NewFactoryImpl(bs.channelGroup).Block(channelID) //创建初始块
}
