...
func LoadTopLevel(configPaths ...string) *TopLevel {
	...
	config.SetEnvPrefix(Prefix) //设置环境变量字母首部
	config.AutomaticEnv()       //设置为true 是允许自动检测环境变量使用变量的值

	replacer := strings.NewReplacer(".", "_")
	config.SetEnvKeyReplacer(replacer) //替换规则

	err := config.ReadInConfig() //读取.yaml配置
	if err != nil {
	...
	var uconf TopLevel
	err = viperutil.EnhancedExactUnmarshal(config, &uconf) //反序列化
	if err != nil {
		logger.Panic("Error unmarshaling config into struct: ", err)
	}

	(&uconf).completeInitialization(filepath.Dir(config.ConfigFileUsed())) //完成对configtx.yaml 文件中Organizations和orderre 补全
...
func (t *TopLevel) completeInitialization(configDir string) {
	for _, org := range t.Organizations {
		org.completeInitialization(configDir) //把 configDir 和configtx.yaml 文件中Organizations.MspDir 拼接新路径 组成msp路径 ==赋值与 org.MspDir
	}

	if t.Orderer != nil {
		t.Orderer.completeInitialization(configDir) //构建以及查漏补全 configtx.yaml中 orderer模块
	}
}
...
func (org *Organization) completeInitialization(configDir string) {
	...
	if org.MSPType == "" {
		org.MSPType = msp.ProviderTypeToString(msp.FABRIC) //bccsp
	}

	if org.AdminPrincipal == "" {
		org.AdminPrincipal = AdminRoleAdminPrincipal //role.Admin
	}
	translatePaths(configDir, org) //把 configDir org.MspDir 拼接新路径 组成msp路径
}
