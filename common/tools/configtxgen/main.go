...
func doOutputBlock(config *genesisconfig.Profile, channelID string, outputBlock string) error {
	pgen := encoder.New(config) //解析配置文件策略 定制策略规则
	...
	genesisBlock := pgen.GenesisBlockForChannel(channelID) //创建初始区块
...
func doOutputChannelCreateTx(conf, baseProfile *genesisconfig.Profile, channelID string, outputChannelCreateTx string) error {
	...
	var err error
	if baseProfile == nil {
		configtx, err = encoder.MakeChannelCreationTransaction(channelID, nil, conf) //基于通道配置 创建一个交易信封
	} else {
...
func main() {
	...
	var profileConfig *genesisconfig.Profile //命令没有指定configPath 配置文件路径 那么获取环境变量 去读取配置文件
	...
	if inspectBlock != "" {
		if err := doInspectBlock(inspectBlock); err != nil { //读取指定路径块文件(.block) 内容转json输出到os.Stdout
			logger.Fatalf("Error on inspectBlock: %s", err)
		}
	}

	if inspectChannelCreateTx != "" {
		if err := doInspectChannelCreateTx(inspectChannelCreateTx); err != nil { //读取指定路径通道配置文件（.tx） 内容转json输出到os.Stdout
			logger.Fatalf("Error on inspectChannelCreateTx: %s", err)
		}
	}

	if outputAnchorPeersUpdate != "" {
		if err := doOutputAnchorPeersUpdate(profileConfig, channelID, outputAnchorPeersUpdate, asOrg); err != nil { //创建更新锚节点的配置文件
			logger.Fatalf("Error on inspectChannelCreateTx: %s", err)
		}
	}
