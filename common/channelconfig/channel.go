...
func NewChannelConfig(channelGroup *cb.ConfigGroup) (*ChannelConfig, error) {
	...
	if err := DeserializeProtoValuesFromGroup(channelGroup, cc.protos); err != nil { //反序列化所有配置
		return nil, errors.Wrap(err, "failed to deserialize values")
	}

	capabilities := cc.Capabilities() //返回通道可使用功能信息 比如同支持 v1_4_3=true v1_3=false

	if err := cc.Validate(capabilities); err != nil { //验证
		return nil, err
	}
	...
	var err error
	for groupName, group := range channelGroup.Groups { //构建各个模块（应用 ，orderer ，联盟）配置 是块配置中的通道(channel_group)配置组（group）下
		switch groupName {
