...
func NewBundleFromEnvelope(env *cb.Envelope) (*Bundle, error) {
	payload, err := utils.UnmarshalPayload(env.Payload) //都是反序列化块信息到结构中
	if err != nil {
...
func NewBundle(channelID string, config *cb.Config) (*Bundle, error) {
	if err := preValidate(config); err != nil { //验证通道配置是否存在等
		return nil, err
	}

	channelConfig, err := NewChannelConfig(config.ChannelGroup) //构建通道配置  msp管理
	if err != nil {
