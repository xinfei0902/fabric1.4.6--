...
func (sv *StandardValues) initializeProtosStruct(objValue reflect.Value) error {
	...
	numFields := objValue.Elem().NumField() //获取类型包含元素个数
	for i := 0; i < numFields; i++ {
		structField := objType.Elem().Field(i) //struct结构类型才可以  获取指定i字段元素（基本是遍历所有元素值）
		logger.Debugf("Processing field: %s\n", structField.Name)
		switch structField.Type.Kind() {
		case reflect.Ptr:
			fieldPtr := objValue.Elem().Field(i)
			if !fieldPtr.CanSet() { //判断指定的i字段的值是否可以改变  如果为false则下面set()会发生错误
