...
func SetupBCCSPKeystoreConfig(bccspConfig *factory.FactoryOpts, keystoreDir string) *factory.FactoryOpts {
	if bccspConfig == nil {
		bccspConfig = factory.GetDefaultOpts() //默认的配置 基本不会进入
	}

	if bccspConfig.ProviderName == "SW" {
		if bccspConfig.SwOpts == nil {
			bccspConfig.SwOpts = factory.GetDefaultOpts().SwOpts //那默认sw 赋值
		}
...
func GetLocalMspConfigWithType(dir string, bccspConfig *factory.FactoryOpts, ID, mspType string) (*msp.MSPConfig, error) {
	switch mspType {
	case ProviderTypeToString(FABRIC): //值为 bsscp 默认mspType = bsscp
...
func GetLocalMspConfig(dir string, bccspConfig *factory.FactoryOpts, ID string) (*msp.MSPConfig, error) {
	signcertDir := filepath.Join(dir, signcerts)                     //msp/signcerts目录 签名用
	keystoreDir := filepath.Join(dir, keystore)                      //私钥
	bccspConfig = SetupBCCSPKeystoreConfig(bccspConfig, keystoreDir) //设置sw配置

	err := factory.InitFactories(bccspConfig) //初始化bsccp
	if err != nil {
		return nil, errors.WithMessage(err, "could not initialize BCCSP Factories")
	}

	signcert, err := getPemMaterialFromDir(signcertDir) //读取 *cert.pem文件
	...
	*/
	...
	sigid := &msp.SigningIdentityInfo{PublicSigner: signcert[0], PrivateSigner: nil} //签名身份

	return getMspConfig(dir, ID, sigid) //获取msp目录下所有信息 信息赋值结构 并返回msp结构
}
...
func GetVerifyingMspConfig(dir, ID, mspType string) (*msp.MSPConfig, error) {
	switch mspType {
	case ProviderTypeToString(FABRIC): //基本都是bccsp
		return getMspConfig(dir, ID, nil) //获取msp目录下所有证书内容 序列号结构配置结构
	case ProviderTypeToString(IDEMIX):
		return GetIdemixMspConfig(dir, ID)
	default:
		return nil, errors.Errorf("unknown MSP type '%s'", mspType)
...
func getMspConfig(dir string, ID string, sigid *msp.SigningIdentityInfo) (*msp.MSPConfig, error) {
	cacertDir := filepath.Join(dir, cacerts) //以下变量 = 各个目录或文件
	...
	//以下getPemMaterialFromDir（）函数  都是检测目录文件是否存在 存在读取证书 然后解析分类证书类型以及内容转字节
	...
	_, err = os.Stat(configFile) //在证书下是否存在config.yaml文件 当crypto-config.yaml中 EnableNodeOUs: true时会生成此文件
	if err == nil {
		...
		if len(configuration.OrganizationalUnitIdentifiers) > 0 { //一般为空 不进入
		...
		// Prepare NodeOUs
		if configuration.NodeOUs != nil && configuration.NodeOUs.Enable { //进入
			...
			// ClientOU
			if nodeOUs.ClientOuIdentifier != nil { //loadCertificateAt（）函数 拼接路径 读取内容
	...
	fmspconf := &msp.FabricMSPConfig{ //把msp目录下所有内容赋值结构
		Admins:                        admincert,
		RootCerts:                     cacerts,
		IntermediateCerts:             intermediatecerts,
