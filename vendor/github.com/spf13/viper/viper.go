...
func (v *Viper) AddConfigPath(in string) {
	if in != "" {
		absin := absPathify(in) //返回绝对路径
		jww.INFO.Println("adding", absin, "to paths to search")
		if !stringInSlice(absin, v.configPaths) { //绝对路径 是否存在v.configPaths 数组中
...
func (v *Viper) ReadInConfig() error {
	...
	if !stringInSlice(v.getConfigType(), SupportedExts) { //判断找到配置文件后缀是否包含在数组中
		return UnsupportedConfigError(v.getConfigType())
	}

	file, err := ioutil.ReadFile(v.getConfigFile()) //读取配置文件
	if err != nil {
		return err
	}
	...
	return v.unmarshalReader(bytes.NewReader(file), v.config) //读取内容 转map 赋值viper.config
}
...
func (v *Viper) unmarshalReader(in io.Reader, c map[string]interface{}) error {
	return unmarshallConfigReader(in, c, v.getConfigType()) //读取配置文件内容转map
}
...
func (v *Viper) getConfigType() string { //截取文件后缀  返回yaml
	if v.configType != "" {
		return v.configType
	}
...
func (v *Viper) searchInPath(in string) (filename string) { //遍历带有后缀数组 组装成 路径/orderer.数组元素当后缀 检测是否存在
