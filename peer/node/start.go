...
func serve(args []string) error {
	...
	mspType := mgmt.GetLocalMSP().GetType() //GetType()函数实现fabric\msp\mspimpl.go No248 line
	...
	opsSystem := newOperationsSystem() //构建operations 服务 监控指标查看运行健康状态等
	err := opsSystem.Start()
	if err != nil {
	...
	serverConfig, err := peer.GetServerConfig() //tls 配置
	if err != nil {
...
func createSelfSignedData() common2.SignedData {
	...
	msg := make([]byte, 32)
	sig, err := sId.Sign(msg) //实现 fabric\msp\identities.go No.221 line
	if err != nil {
		logger.Panicf("Failed creating self signed data because message signing failed: %v", err)
	}
	peerIdentity, err := sId.Serialize() //实现 fabric\msp\identities.go No.174 line
	if err != nil {
