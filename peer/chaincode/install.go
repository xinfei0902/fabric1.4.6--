...
func installCmd(cf *ChaincodeCmdFactory) *cobra.Command {
	...
	flagList := []string{ //命令指定的参数 可以从命令获取变量信息
		"lang",
		"ctor",
		"path",
		"name",
...
func install(msg proto.Message, cf *ChaincodeCmdFactory) error {
	...
	prop, _, err := utils.CreateInstallProposalFromCDS(msg, creator) //创建提案
	if err != nil {
		return fmt.Errorf("Error creating proposal  %s: %s", chainFuncName, err)
	}

	var signedProp *pb.SignedProposal
	signedProp, err = utils.GetSignedProposal(prop, cf.Signer) //对提案签署
	if err != nil {
		return fmt.Errorf("Error creating signed proposal  %s: %s", chainFuncName, err)
	}

	// install is currently only supported for one peer
	proposalResponse, err := cf.EndorserClients[0].ProcessProposal(context.Background(), signedProp) //返回提案结果
	if err != nil {
...
func genChaincodeDeploymentSpec(cmd *cobra.Command, chaincodeName, chaincodeVersion string) (*pb.ChaincodeDeploymentSpec, error) {
	if existed, _ := ccprovider.ChaincodePackageExists(chaincodeName, chaincodeVersion); existed { //检测合约是否存在
		return nil, fmt.Errorf("chaincode %s:%s already exists", chaincodeName, chaincodeVersion)
	}

	spec, err := getChaincodeSpec(cmd) //获取组装链码信息
	if err != nil {
		return nil, err
	}

	cds, err := getChaincodeDeploymentSpec(spec, true) //获取链码部署规范
	if err != nil {
...
func getPackageFromFile(ccpackfile string) (proto.Message, *pb.ChaincodeDeploymentSpec, error) {
	...
	o := ccpack.GetPackageObject() //看ccpack是CDSPackage还是SignedCDSPackage 如果是CDSPackage 实现：fabric\core\common\ccprovider\cdspackage.go No.107 line 如果是SignedCDSPackage 实现abric\core\common\ccprovider\sigcdspackage.go No.120 line
...
func chaincodeInstall(cmd *cobra.Command, ccpackfile string, cf *ChaincodeCmdFactory) error {
	...
	if ccpackfile == "" {
		...
		ccpackmsg, err = genChaincodeDeploymentSpec(cmd, chaincodeName, chaincodeVersion) //读取chaincode源码文件，并打包
		if err != nil {
			return err
		}
	} else {
		...
		ccpackmsg, cds, err = getPackageFromFile(ccpackfile) //指定已经打包好的链码
