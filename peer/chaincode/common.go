...
func getChaincodeSpec(cmd *cobra.Command) (*pb.ChaincodeSpec, error) {
	...
	if err := checkChaincodeCmdParams(cmd); err != nil { //检测合约请求参数
	...
	chaincodeLang = strings.ToUpper(chaincodeLang) //获取参数中 -l 指向语言
	spec = &pb.ChaincodeSpec{
		Type:        pb.ChaincodeSpec_Type(pb.ChaincodeSpec_Type_value[chaincodeLang]), //1 golang
...
func chaincodeInvokeOrQuery(cmd *cobra.Command, invoke bool, cf *ChaincodeCmdFactory) (err error) {
	spec, err := getChaincodeSpec(cmd) //获取链码请求规范
	if err != nil {
		return err
	}
...
func validatePeerConnectionParameters(cmdName string) error {
	if connectionProfile != common.UndefinedParamValue { //peer请求是否指定参数  一般为空
...
func ChaincodeInvokeOrQuery(
	...
	prop, txid, err := putils.CreateChaincodeProposalWithTxIDAndTransient(pcommon.HeaderType_ENDORSER_TRANSACTION, cID, invocation, creator, txID, tMap) //返回提案和交易id
	if err != nil {
		return nil, errors.WithMessage(err, fmt.Sprintf("error creating proposal for %s", funcName))
	}

	signedProp, err := putils.GetSignedProposal(prop, signer) //获取签署的提案
	if err != nil {
		return nil, errors.WithMessage(err, fmt.Sprintf("error creating signed proposal for %s", funcName))
	}
	var responses []*pb.ProposalResponse
	for _, endorser := range endorserClients { //发送到背书节点
	...
	if invoke {
		if proposalResp != nil {
			...
			env, err := putils.CreateSignedTx(prop, signer, responses...) //创建信封
			if err != nil {
