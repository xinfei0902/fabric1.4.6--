...
func update(cmd *cobra.Command, args []string, cf *ChannelCmdFactory) error {
	...
	fileData, err := ioutil.ReadFile(channelTxFile) //读取要更新配置文件
	if err != nil {
		return ConfigTxFileNotFound(err.Error())
	}

	ctxEnv, err := utils.UnmarshalEnvelope(fileData) //做成信封
	if err != nil {
		return err
	}

	sCtxEnv, err := sanityCheckAndSignConfigTx(ctxEnv) //检测签署信封
	if err != nil {
		return err
	}
	...
	err = broadcastClient.Send(sCtxEnv) //把信封信息发送orderer
	if err != nil {
		return err
	}
