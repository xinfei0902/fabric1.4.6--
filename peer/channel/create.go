...
func sendCreateChainTransaction(cf *ChannelCmdFactory) error {
	...
	if chCrtEnv, err = sanityCheckAndSignConfigTx(chCrtEnv); err != nil { //检测读取文件 然后用读取内容组装交易（信封）
		return err
	}
	...
	err = broadcastClient.Send(chCrtEnv) //把交易（信封）发送orderer

	return err
}

func executeCreate(cf *ChannelCmdFactory) error {
	err := sendCreateChainTransaction(cf) //读取通道tx文件 根据内容组装交易（信封） 发送到orderer
	if err != nil {
		return err
	}

	block, err := getGenesisBlock(cf) //deliver客户端 获取第零个块（创世区块）
	if err != nil {
		return err
	}
	...
	err = ioutil.WriteFile(file, b, 0644) //本地创建 通道名.block文件
	if err != nil {
		return err
	}
...
func create(cmd *cobra.Command, args []string, cf *ChannelCmdFactory) error {
	...
	return executeCreate(cf) //执行创建通道
}
